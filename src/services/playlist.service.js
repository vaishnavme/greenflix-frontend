...
export const createPlaylist = async ({ playlistName, video, dispatch }) => {
    try {
        const response = await axios.post(
            `${BASE_URI}/playlist/create/${video._id}`,
            {
                playlistName
            }
        );
...
export const removeFromPlaylist = async ({ playListId, video, dispatch }) => {
    try {
        const {
            ...
            `${BASE_URI}/playlist/${playListId}/${video._id}`
        );
        dispatch({
...
export const addToPlaylist = async ({ playListId, video, dispatch }) => {
    try {
        const {
            data: { playlistId }
        } = await axios.post(`${BASE_URI}/playlist/${playListId}/${video._id}`);
...
export const deleteUserPlaylist = async ({ playListId, dispatch }) => {
    try {
        const {
            data: { playlistId }
        } = await axios.delete(`${BASE_URI}/playlist/${playListId}`);
